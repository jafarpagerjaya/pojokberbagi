...
CREATE TABLE akun (
    ...
    CONSTRAINT U_EMAIL_AKUN UNIQUE(email),
    CONSTRAINT U_KONTAK_AKUN UNIQUE(kontak),
...
CREATE TABLE artikel (
    id_artikel INT UNSIGNED NOT NULL AUTO_INCREMENT PRIMARY KEY,
    judul VARCHAR(50) NOT NULL,
    isi TEXT,
    id_author SMALLINT UNSIGNED,
    id_editor SMALLINT UNSIGNED,
    id_gambar_small INT UNSIGNED,
    id_gambar_wide INT UNSIGNED,
    aktif ENUM('0','1') DEFAULT '1',
    publish_at TIMESTAMP,
    create_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    modified_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    CONSTRAINT F_ID_AUTHOR_ARTIKEL_ODN FOREIGN KEY(id_author) REFERENCES pegawai(id_pegawai) ON DELETE SET NULL ON UPDATE CASCADE,
    CONSTRAINT F_ID_EDITOR_ARTIKEL_ODN FOREIGN KEY(id_editor) REFERENCES pegawai(id_pegawai) ON DELETE SET NULL ON UPDATE CASCADE,
    CONSTRAINT F_ID_GAMBAR_SMALL_ARTIKEL_ODN FOREIGN KEY(id_gambar_small) REFERENCES gambar(id_gambar) ON DELETE SET NULL ON UPDATE CASCADE,
    CONSTRAINT F_ID_GAMBAR_WIDE_ARTIKEL_ODN FOREIGN KEY(id_gambar_wide) REFERENCES gambar(id_gambar) ON DELETE SET NULL ON UPDATE CASCADE
    CONSTRAINT U_JUDUL_ARTIKEL UNIQUE(judul)
)ENGINE=INNODB;

CREATE TABLE kunjungan_artikel (
    id_artikel INT UNSIGNED,
    id_pengunjung INT UNSIGNED,
    create_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
)ENGINE=INNODB;

CREATE TABLE list_gambar_artikel (
    id_artikel INT UNSIGNED,
    id_gambar INT UNSIGNED,
    create_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    modified_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    CONSTRAINT F_ID_ARTIKEL_LGA_ODN FOREIGN KEY(id_artikel) REFERENCES artikel(id_artikel) ON DELETE SET NULL ON UPDATE CASCADE,
    CONSTRAINT F_ID_GAMBAR_LGA_ODC FOREIGN KEY(id_gambar) REFERENCES gambar(id_gambar) ON DELETE CASCADE ON UPDATE CASCADE
)ENGINE=INNODB;
...
CREATE TABLE marketing (
    ...
    CONSTRAINT UN_NAMA_MARKETING UNIQUE(nama)
)ENGINE=INNODB;
...
CREATE TABLE order_donasi (
    ...
    id_marketing SMALLINT UNSIGNED,
    ...
    CONSTRAINT F_ID_MARKETING_ORDER_DONASI_ODN FOREIGN KEY(id_marketing) REFERENCES marketing(id_marketing) ON DELETE SET NULL ON UPDATE CASCADE
)ENGINE=INNODB;

CREATE TABLE order_va (
    id_order_va SMALLINT UNSIGNED AUTO_INCREMENT NOT NULL PRIMARY KEY,
    account_number VARCHAR(100) NOT NULL,
    id_order_donasi INT UNSIGNED,
    create_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    modified_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    CONSTRAINT F_ID_ORDER_DONASI_ORDER_VA_ODC FOREIGN KEY(id_order_donasi) REFERENCES order_donasi(id_order_donasi) ON DELETE CASCADE ON UPDATE CASCADE
)ENGINE=INNODB;

CREATE TABLE order_qr (
    id_order_qr SMALLINT UNSIGNED AUTO_INCREMENT NOT NULL PRIMARY KEY,
    qr_code TEXT NOT NULL,
    id_order_donasi INT UNSIGNED,
    create_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    modified_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    CONSTRAINT F_ID_ORDER_DONASI_ORDER_QR_ODC FOREIGN KEY(id_order_donasi) REFERENCES order_donasi(id_order_donasi) ON DELETE CASCADE ON UPDATE CASCADE
)ENGINE=INNODB;
...
CREATE TABLE donasi (
    ...
    CONSTRAINT F_ID_MARKETING_DONASI_ODN FOREIGN KEY(id_marketing) REFERENCES marketing(id_marketing) ON DELETE SET NULL ON UPDATE CASCADE
)ENGINE=INNODB;
...
AFTER INSERT ON order_paygate FOR EACH ROW
    BEGIN
    	IF NEW.status = 'SUCCESSFUL' THEN
        	INSERT INTO donasi(id_bantuan, id_donatur, id_cp, kode_pembayaran, alias, kontak, email, doa, jumlah_donasi, bayar, waktu_bayar, notifikasi, id_order_paygate, id_marketing)
        	SELECT id_bantuan, id_donatur, id_cp, kode_pembayaran, alias, kontak, email, doa, jumlah_donasi, '1', NEW.completed_at, notifikasi, NEW.id_order_paygate, id_marketing
