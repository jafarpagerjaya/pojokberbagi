...
INSERT INTO penyelenggara_jasa_pembayaran(nama, kode) VALUES
...
CREATE TABLE pengadaan(
    id_pengadaan BIGINT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
    nominal BIGINT UNSIGNED,
    saldo_penarikan BIGINT UNSIGNED,
    saldo_rab INT UNSIGNED,
    id_petugas SMALLINT UNSIGNED,
    id_penarikan INT UNSIGNED,
    id_rab BIGINT UNSIGNED,
    create_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    modified_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    CONSTRAINT F_ID_PENARIKAN_PENGADAAN_ODR FOREIGN KEY(id_penarikan) REFERENCES penarikan(id_penarikan) ON DELETE RESTRICT ON UPDATE CASCADE,
    CONSTRAINT F_ID_RAB_PENGADAAN_ODC FOREIGN KEY(id_rab) REFERENCES rencana_anggaran_belanja(id_rab) ON DELETE CASCADE ON UPDATE CASCADE,
    CONSTRAINT F_ID_PETUGAS_PENGADAAN_ODR FOREIGN KEY(id_petugas) REFERENCES pegawai(id_pegawai) ON DELETE RESTRICT ON UPDATE CASCADE
)ENGINE=INNODB;
...
CREATE TABLE belanja(
    ...
    CONSTRAINT F_ID_RAB_BELANJA_ODR FOREIGN KEY(id_rab) REFERENCES rencana_anggaran_belanja(id_rab) ON DELETE RESTRICT ON UPDATE CASCADE
)ENGINE=INNODB;
...
-- INSERT INTO pelaksanaan(deskripsi,jumlah_pelaksanaan,total_anggaran,status,create_at,id_bantuan) VALUES('Program Kurban BJB 2022', 43, 1185000000, 'S', '2022-07-09',(SELECT id_bantuan FROM bantuan WHERE nama = 'RAIH SURGA BERSAMA POJOK QURBAN'));
...
-- ALTER TABLE pelaksanaan MODIFY status ENUM('P','J','S') DEFAULT 'P';

-- ALTER TABLE akun ADD CONSTRAINT NU_EMAIL_AKUN UNIQUE(email);

-- ALTER TABLE donatur ADD COLUMN jenis_kelamin ENUM('P','L') after nama;

-- INSERT INTO pelaksanaan(jumlah_pelaksanaan,total_anggaran,status,create_at) VALUES(25000,3125000000,'S','2022-03-18');
...
-- CREATE TABLE diskusi_permohonan_rencana ( id_diskusi_permohonan_rencana int unsigned NOT NULL AUTO_INCREMENT, id_ruang_diskusi int unsigned DEFAULT NULL, id_akun int unsigned DEFAULT NULL, pesan text NOT NULL, dibaca enum('1') DEFAULT NULL, create_at timestamp NULL DEFAULT CURRENT_TIMESTAMP, modified_at timestamp NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP, PRIMARY KEY (id_diskusi_permohonan_rencana), KEY F_ID_RUANG_DISKUSI_DPR_ODC (id_ruang_diskusi), KEY F_ID_AKUN_DPR_ODN (id_akun), CONSTRAINT F_ID_AKUN_DPR_ODN FOREIGN KEY (id_akun) REFERENCES akun (id_akun) ON DELETE SET NULL ON UPDATE CASCADE, CONSTRAINT F_ID_RUANG_DISKUSI_DPR_ODC FOREIGN KEY (id_ruang_diskusi) REFERENCES ruang_diskusi_permohonan_rencana (id_ruang_diskusi) ON DELETE CASCADE ON UPDATE CASCADE ) ENGINE=InnoDB;
...
-- INSERT INTO rencana(id_pegawai,total_anggaran,create_at,status,keterangan)
-- SELECT 1, total_anggaran, create_at, 'SD', deskripsi FROM pelaksanaan WHERE id_pelaksanaan < 6 ORDER BY CREATE_AT;
-- UPDATE rencana SET id_bantuan = 1 WHERE id_rencana IN ('1','2','4') AND LOWER(keterangan) LIKE '%sembako bjb%';
-- UPDATE rencana SET id_bantuan = 2 WHERE id_rencana IN ('3') AND LOWER(keterangan) LIKE '%geberr%';
-- UPDATE rencana SET id_bantuan = 13 WHERE LOWER(keterangan) LIKE '%kurban bjb%';
...
-- ALTER TABLE anggaran_pelaksanaan_donasi ADD COLUMN id_apd BIGINT UNSIGNED AUTO_INCREMENT PRIMARY KEY FIRST;
-- ALTER TABLE anggaran_pelaksanaan_donasi DROP FOREIGN KEY F_ID_KEBUTUHAN_ANGGARAN_KEBUTUHAN_DONASI_ODN;
-- ALTER TABLE anggaran_pelaksanaan_donasi DROP INDEX F_ID_KEBUTUHAN_ANGGARAN_KEBUTUHAN_DONASI_ODN;
-- ALTER TABLE anggaran_pelaksanaan_donasi ADD CONSTRAINT F_ID_KEBUTUHAN_ANGGARAN_PELAKSANAAN_DONASI_ODR FOREIGN KEY(id_kebutuhan) REFERENCES kebutuhan(id_kebutuhan) ON DELETE RESTRICT ON UPDATE CASCADE;
-- ALTER TABLE anggaran_pelaksanaan_donasi DROP FOREIGN KEY F_ID_PELAKSANAAN_ANGGARAN_KEBUTUHAN_DONASI_ODR;
-- ALTER TABLE anggaran_pelaksanaan_donasi DROP INDEX F_ID_PELAKSANAAN_ANGGARAN_KEBUTUHAN_DONASI_ODR;
-- ALTER TABLE anggaran_pelaksanaan_donasi ADD CONSTRAINT F_ID_PELAKSANAAN_ANGGARAN_PELAKSANAAN_DONASI_ODR FOREIGN KEY(id_pelaksanaan) REFERENCES pelaksanaan(id_pelaksanaan) ON DELETE RESTRICT ON UPDATE CASCADE;
-- ALTER TABLE anggaran_pelaksanaan_donasi DROP FOREIGN KEY F_ID_DONASI_ANGGARAN_KEBUTUHAN_DONASI_ODC;
-- ALTER TABLE anggaran_pelaksanaan_donasi DROP INDEX F_ID_DONASI_ANGGARAN_KEBUTUHAN_DONASI_ODC;
-- ALTER TABLE anggaran_pelaksanaan_donasi ADD CONSTRAINT F_ID_DONASI_ANGGARAN_PELAKSANAAN_DONASI_ODC FOREIGN KEY(id_donasi) REFERENCES donasi(id_donasi) ON DELETE CASCADE ON UPDATE CASCADE;
-- ALTER TABLE anggaran_pelaksanaan_donasi MODIFY nominal_kebutuhan BIGINT UNSIGNED NOT NULL;
-- ALTER TABLE anggaran_pelaksanaan_donasi MODIFY saldo_kebutuhan BIGINT UNSIGNED NOT NULL;
...
INSERT INTO anggaran_pelaksanaan_donasi (id_donasi, nominal_kebutuhan, nominal_penggunaan_donasi, saldo_kebutuhan, saldo_donasi, id_kebutuhan, keterangan, id_pelaksanaan)
SELECT id_donasi, jumlah_donasi, jumlah_donasi, 0, 0, 1, 'TOTAL RAB', (SELECT id_pelaksanaan FROM pelaksanaan WHERE create_at = '2022-03-18') FROM donasi LEFT JOIN anggaran_pelaksanaan_donasi apd USING(id_donasi) WHERE id_bantuan = 1 AND bayar = 1 AND apd.id_donasi IS NULL;

-- ALTER TABLE pelaksanaan ADD COLUMN id_rencana INT UNSIGNED AFTER status;
-- ALTER TABLE pelaksanaan ADD CONSTRAINT F_ID_RENCANA_PELAKSANAAN_ODR FOREIGN KEY(id_rencana) REFERENCES rencana(id_rencana) ON DELETE RESTRICT ON UPDATE CASCADE;

-- ALTER TABLE pelaksanaan MODIFY total_anggaran BIGINT UNSIGNED NOT NULL DEFAULT 0;

-- ALTER TABLE pelaksanaan ADD COLUMN tanggal_eksekusi DATE AFTER status;
-- UPDATE pelaksanaan SET tanggal_eksekusi = DATE_FORMAT(create_at, '%Y-%m-%d');

-- UPDATE pelaksanaan p JOIN rencana r USING(id_rencana) SET p.id_rencana = r.id_rencana WHERE p.create_at = r.create_at AND p.id_pelaksanaan = r.id_rencana;
...
-- -- INSERT INI DIGUNAKAN JIKA relasi pencairan dengan pelaksananan 1 ke 1
-- INSERT INTO pencairan(total) 
-- SELECT total_anggaran FROM pelaksanaan WHERE id_pelaksanaan NOT IN (SELECT id_pelaksanaan FROM pencairan);
-- -- 
...
-- xxx UPDATE pencairan p1 JOIN pelaksanaan p2 USING(id_pelaksanaan) SET p1.create_at = p2.create_at;
...
-- INSERT INTO penyelenggara_jasa_pembayaran(nama, kode) 
-- SELECT nama, kode FROM channel_payment WHERE jenis IN ('TB');
-- INSERT INTO penyelenggara_jasa_pembayaran(nama) VALUES
-- ('PT Espay Debit Indonesia Koe'),
-- ('PT Dompet Anak Bangsa');
-- UPDATE penyelenggara_jasa_pembayaran SET nama = 'PT Bank Rakyat Indonesia (Persero), Tbk' WHERE kode = '002'; 
-- UPDATE penyelenggara_jasa_pembayaran SET nama = 'PT Bank Jabar Dan Banten' WHERE kode = '110'; 
-- UPDATE penyelenggara_jasa_pembayaran SET nama = 'PT Bank Syariah Indonesia' WHERE kode = '451'; 
-- UPDATE penyelenggara_jasa_pembayaran SET kode = 'DAN' WHERE nama = 'PT Espay Debit Indonesia Koe';
-- UPDATE penyelenggara_jasa_pembayaran SET kode = 'GOP' WHERE nama = 'PT Dompet Anak Bangsa';
...
-- INSERT INTO channel_account(nama, nomor, atas_nama)
-- SELECT nama, nomor, atas_nama 
-- FROM channel_payment WHERE jenis NOT IN('GI','QR') ORDER BY kode;
...
-- UPDATE channel_payment SET kode = 'DAN' WHERE LOWER(nama) = 'dana';
-- UPDATE channel_payment SET kode = 'GOP' WHERE LOWER(nama) = 'gopay';
...
UPDATE channel_account SET jenis = 'EW' WHERE id_pjp IS NOT NULL AND nama NOT LIKE 'Bank%' AND LOWER(nama) NOT LIKE '%kantor%';
UPDATE channel_account SET jenis = 'KT' WHERE LOWER(nama) LIKE '%kantor%';

-- MENESUAIKAN CP
-- ALTER TABLE channel_payment ADD COLUMN id_ca TINYINT UNSIGNED AFTER modified_at;
-- ALTER TABLE channel_payment ADD CONSTRAINT F_ID_CA_CHANNEL_PAYMENT_ODR FOREIGN KEY(id_ca) REFERENCES channel_account(id_ca) ON DELETE RESTRICT ON UPDATE CASCADE;
UPDATE channel_payment cp, channel_account ca LEFT JOIN penyelenggara_jasa_pembayaran pjp USING(id_pjp) SET cp.id_ca = ca.id_ca WHERE cp.nama LIKE CONCAT('%',ca.nama,'%');
...
-- DROP PROCEDURE VirtualCAPenarikan;
-- DELIMITER $$
-- CREATE PROCEDURE VirtualCAPenarikan(IN in_id_penarikan INT, IN in_id_ca TINYINT)
--     VCAPenarikanLabel:BEGIN
--         -- Menampung State Cursor
--         DECLARE finished_vca_reduce TINYINT DEFAULT 0;
--         -- Var untuk menampung isi cursor
...
-- ALTER TABLE kebutuhan ADD COLUMN id_kk TINYINT UNSIGNED;
-- ALTER TABLE kebutuhan ADD CONSTRAINT F_ID_KK_KEBUTUHAN_ODN FOREIGN KEY(id_kk) REFERENCES kategori_kebutuhan(id_kk) ON DELETE SET NULL ON UPDATE CASCADE;
...
-- M = Masuk, K = Keluar, P = Pindah

-- INSERT INTO channel_account(nama,nomor,atas_nama)
-- SELECT nama, nomor, atas_nama, (
-- SELECT SUM(d.jumlah_donasi)
-- FROM donasi d LEFT JOIN anggaran_pelaksanaan_donasi a USING(id_donasi) LEFT JOIN channel_payment c USING(id_cp)
-- WHERE d.bayar = 1 AND a.id_pelaksanaan IS NULL AND channel_payment.kode = c.kode
-- GROUP BY c.kode
-- ORDER BY c.kode
-- ) saldo
-- FROM channel_payment WHERE jenis NOT IN('GI','QR') ORDER BY kode;
