...
INSERT INTO rencana(id_pembuat,total_anggaran,create_at,status)
SELECT 1, total_anggaran, create_at, 'SD' FROM pelaksanaan WHERE id_pelaksanaan < 6;
UPDATE rencana SET id_bantuan = 1 WHERE id_rencana IN ('1','2','4') AND LOWER(deskripsi) LIKE '%sembako bjb%';
UPDATE rencana SET id_bantuan = 2 WHERE id_rencana IN ('3') AND LOWER(deskripsi) LIKE '%geberr%';
UPDATE rencana SET id_bantuan = (SELECT id_bantuan FROM bantuan WHERE nama = 'RAIH SURGA BERSAMA POJOK QURBAN') WHERE LOWER(deskripsi) LIKE '%kurban bjb%';
...
INSERT INTO anggaran_pelaksanaan_donasi (id_donasi, nominal_kebutuhan, nominal_penggunaan_donasi, saldo_kebutuhan, saldo_donasi, id_kebutuhan, keterangan, id_pelaksanaan) VALUES
((SELECT id_donasi, jumlah_donasi, jumlah_donasi FROM donasi LEFT JOIN anggaran_pelaksanaan_donasi apd USING(id_donasi) WHERE id_bantuan = 1 AND bayar = 1 AND apd.id_donasi IS NULL), 0, 0, 1, 'TOTAL RAB', (SELECT id_pelaksanaan FROM pelaksanaan WHERE create_at = '2022-03-18'));
...
UPDATE pencairan p1 JOIN pelaksanaan p2 USING(id_pelaksanaan) SET p1.create_at = p2.create_at;
...
SELECT IFNULL(MIN(a.saldo_donasi), d.jumlah_donasi), d.id_donasi, cp.id_ca, d.waktu_bayar FROM donasi d JOIN channel_payment cp USING(id_cp) LEFT JOIN anggaran_pelaksanaan_donasi a USING(id_donasi) WHERE d.bayar = 1 GROUP BY d.id_donasi ORDER BY id_donasi;
...
UPDATE channel_account SET jenis = 'EW' WHERE id_pjp IS NOT NULL AND nama NOT LIKE 'Bank%';
UPDATE channel_account SET jenis = 'KT' WHERE id_pjp IS NULL AND LOWER(nama) LIKE '%kantor%';
...
ALTER TABEL kebutuhan ADD COLUMN id_kk TINYINT UNSIGNED;
